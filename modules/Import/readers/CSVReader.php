...
class Import_CSV_Reader extends Import_File_Reader {
	...
	public function read() {
        ...
        // SalesPlatform.ru begin sets locale information
        function setLocaleEncoding() {
                $file_encoding = $this->userInputObject->get('file_encoding');
                $current_locale = setLocale(LC_ALL, null);
                $prefix_locale = substr($current_locale, 0, strrpos($current_locale, '.'));
                if (strpos($file_encoding, "-")) {
                    $additional_locale = str_replace("-", "", $file_encoding);
                }
                                 
                if (!empty($prefix_locale)) {
                    if (isset($additional_locale)) {
                        if (!setLocale(LC_ALL, $prefix_locale.'.'.$file_encoding, $prefix_locale.'.'.$additional_locale, $file_encoding, $additional_locale)) {
                                return false;
                        } else {
                                return true;
                        }
                    } else {
                        if (!setLocale(LC_ALL, $prefix_locale.'.'.$file_encoding, $file_encoding)) {
                                return false;   
                        } else {
                                return true;
                        }
                    ...
                    if (isset($additional_locale)) {
                        if (!setLocale(LC_ALL, $file_encoding, $additional_locale)) {
                                return false;
                        } else {
                                return true;      
                        }
                } else {
                        if (!setLocale(LC_ALL, $file_encoding)) {
                                return false;
                        } else {
                                return true;
                        }
