...
class SMSNotifier extends SMSNotifierBase {
	...
	static function sendsms($message, $tonumbers, $ownerid = false, $linktoids = false, $linktoModule = false) {
		...
		if($linktoids !== false) {
			...
			if($linktoModule !== false) {
				relateEntities($focus, $moduleName, $focus->id, $linktoModule, $linktoids);
                                ...
                                for($i = 0; $i < count($tonumbers) && $i < count($linktoids); $i++) {
                                    $processedMessage = getMergedDescription($message, $linktoids[$i], $linktoModule);
				    $responses = self::fireSendSMS($processedMessage, array($tonumbers[$i]));
				...
				if($linkidsetypes && $adb->num_rows($linkidsetypes)) {
					while($linkidsetypesrow = $adb->fetch_array($linkidsetypes)) {
						relateEntities($focus, $moduleName, $focus->id, $linkidsetypesrow['setype'], $linkidsetypesrow['crmid']);
                                                // SalesPlatform.ru begin: Add last SMS date
                                                self::updateLastSMSDate($linkidsetypesrow['setype'], $linkidsetypesrow['crmid']);
                                                // SalesPlatform.ru end
					}
				}
            			// SalesPlatform.ru begin: Add SMS message template processing
        ...
        // SalesPlatform.ru begin: Add last SMS date
        static function updateLastSMSDate($moduleName, $crmids) {
            global $adb;
            $rel_focus = CRMEntity::getInstance($moduleName);
            if(!is_array($crmids)) $crmids = Array($crmids);

            $adb->pquery('UPDATE ' . $rel_focus->table_name . ' SET splastsms=Now() WHERE ' . $rel_focus->table_index . ' IN (' . generateQuestionMarks($crmids) . ')',
                    array($crmids));
        }
        // SalesPlatform.ru end
		...
		if(empty($id)) {
			$adb->pquery("INSERT INTO vtiger_smsnotifier_servers (providertype,username,password,isactive,parameters) VALUES(?,?,?,?,?)",
				array($providertype, $username, $password, $isactive, $parameters));
                        // SalesPlatform.ru begin active sms-provider
			...
			$adb->pquery("UPDATE vtiger_smsnotifier_servers SET username=?, password=?, isactive=?, providertype=?, parameters=? WHERE id=?",
                ...
                // SalesPlatform.ru begin active sms-provider
                if ($isactive == 1) {
                     $adb->pquery("UPDATE vtiger_smsnotifier_servers SET isactive = 0 WHERE id != ?", array($id));
                }
                // SalesPlatform.ru end
	}
